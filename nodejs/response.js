...
const response = {
  ...
  serverError: (res, error = 'Server Error') => (isProd ? res.sendStatus(500) : res.status(500).send({ errors: [error] })),
  loginRequired: (res, error = 'You must be logged in to access this resource') => res.status(401).send({ errors: [error] }),
